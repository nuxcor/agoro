...
- **Remote-first navigation** — a top tab bar carries the destinations; BACK steps focus up to it and OK or DOWN commits, and every list takes channel numbers straight off the keypad. In the player OK opens the channel options (favorite and more), LEFT opens the channel list, RIGHT opens the controls — every long-press action also has a plain-key route for remotes without key repeat
